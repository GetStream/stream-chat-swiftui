...
/// Protocol that can be adopted by factories still implementing the legacy Stream Chat SwiftUI API.
/// It inherits from the new `ViewFactory` and exposes the previous method signatures so existing
/// implementations can keep compiling without any changes.
@MainActor public protocol ViewFactoryMigration: ViewFactory {
...
// MARK: - Channel List Bridging

extension ViewFactoryMigration {
    public func makeChannelListHeaderViewModifier(options: ChannelListHeaderViewModifierOptions) -> HeaderViewModifier {
        makeChannelListHeaderViewModifier(title: options.title)
    }

    public func makeNoChannelsView(options: NoChannelsViewOptions) -> NoChannels {
        makeNoChannelsView()
    }

    public func makeLoadingView(options: LoadingViewOptions) -> LoadingContent {
        makeLoadingView()
    }

    public func makeChannelListItem(options: ChannelListItemOptions<ChannelDestination>) -> ChannelListItemType {
        makeChannelListItem(
            ...
            trailingSwipeRightButtonTapped: options.trailingSwipeRightButtonTapped,
            trailingSwipeLeftButtonTapped: options.trailingSwipeLeftButtonTapped,
            leadingSwipeButtonTapped: options.leadingSwipeButtonTapped
        )
    }

    public func makeChannelAvatarView(options: ChannelAvatarViewFactoryOptions) -> ChannelAvatarViewType {
        makeChannelAvatarView(for: options.channel, with: options.options)
    }

    public func makeChannelListBackground(options: ChannelListBackgroundOptions) -> ChannelListBackground {
        makeChannelListBackground(colors: options.colors)
    }

    public func makeChannelListItemBackground(options: ChannelListItemBackgroundOptions) -> ChannelListItemBackground {
        makeChannelListItemBackground(channel: options.channel, isSelected: options.isSelected)
    }

    public func makeChannelListDividerItem(options: ChannelListDividerItemOptions) -> ChannelListDividerItem {
        makeChannelListDividerItem()
    }

    public func makeMoreChannelActionsView(options: MoreChannelActionsViewOptions) -> MoreActionsView {
        makeMoreChannelActionsView(
            for: options.channel,
            swipedChannelId: options.swipedChannelId,
    ...
    public func makeTrailingSwipeActionsView(options: TrailingSwipeActionsViewOptions) -> TrailingSwipeActionsViewType {
        makeTrailingSwipeActionsView(
            channel: options.channel,
    ...
    public func makeLeadingSwipeActionsView(options: LeadingSwipeActionsViewOptions) -> LeadingSwipeActionsViewType {
        makeLeadingSwipeActionsView(
            channel: options.channel,
            offsetX: options.offsetX,
            buttonWidth: options.buttonWidth,
            swipedChannelId: options.swipedChannelId,
            buttonTapped: options.buttonTapped
        )
    }

    public func makeChannelListTopView(options: ChannelListTopViewOptions) -> ChannelListTopViewType {
        makeChannelListTopView(searchText: options.searchText)
    }

    public func makeChannelListFooterView(options: ChannelListFooterViewOptions) -> ChannelListFooterViewType {
        makeChannelListFooterView()
    }

    public func makeChannelListStickyFooterView(options: ChannelListStickyFooterViewOptions) -> ChannelListStickyFooterViewType {
        makeChannelListStickyFooterView()
    }

    public func makeSearchResultsView(options: SearchResultsViewOptions) -> ChannelListSearchResultsViewType {
    ...
    public func makeChannelListContentModifier(options: ChannelListContentModifierOptions) -> ChannelListContentModifier {
        makeChannelListContentModifier()
    }

    public func makeChannelListModifier(options: ChannelListModifierOptions) -> ChannelListModifier {
        makeChannelListModifier()
    }
}

// MARK: - Destinations & Message List Setup

extension ViewFactoryMigration {
    public func makeChannelDestination(options: ChannelDestinationOptions) -> @MainActor (ChannelSelectionInfo) -> ChannelDestination {
        makeChannelDestination()
    }

    public func makeMessageThreadDestination(options: MessageThreadDestinationOptions) -> @MainActor (ChatChannel, ChatMessage) -> MessageThreadDestination {
        makeMessageThreadDestination()
    }

    public func makeEmptyMessagesView(options: EmptyMessagesViewOptions) -> EmptyMessagesViewType {
        makeEmptyMessagesView(for: options.channel, colors: options.colors)
    }

    public func makeMessageListModifier(options: MessageListModifierOptions) -> MessageListModifier {
        makeMessageListModifier()
    }

    public func makeMessageListContainerModifier(options: MessageListContainerModifierOptions) -> MessageListContainerModifier {
        makeMessageListContainerModifier()
    }

    public func makeMessageAvatarView(options: MessageAvatarViewOptions) -> UserAvatar {
        makeMessageAvatarView(for: options.userDisplayInfo)
    }

    public func makeQuotedMessageAvatarView(options: QuotedMessageAvatarViewOptions) -> QuotedUserAvatar {
        makeQuotedMessageAvatarView(for: options.userDisplayInfo, size: options.size)
    }

    public func makeChannelHeaderViewModifier(options: ChannelHeaderViewModifierOptions) -> ChatHeaderViewModifier {
        makeChannelHeaderViewModifier(for: options.channel)
    }

    public func makeChannelBarsVisibilityViewModifier(options: ChannelBarsVisibilityViewModifierOptions) -> ChangeBarsVisibilityModifier {
        makeChannelBarsVisibilityViewModifier(shouldShow: options.shouldShow)
    }

    public func makeChannelLoadingView(options: ChannelLoadingViewOptions) -> ChannelLoadingViewType {
        makeChannelLoadingView()
    }

    public func makeMessageThreadHeaderViewModifier(options: MessageThreadHeaderViewModifierOptions) -> ThreadHeaderViewModifier {
        makeMessageThreadHeaderViewModifier()
    }

    public func makeMessageListBackground(options: MessageListBackgroundOptions) -> MessageListBackground {
        makeMessageListBackground(colors: options.colors, isInThread: options.isInThread)
    }
}

// MARK: - Message Content & Attachments

extension ViewFactoryMigration {
    public func makeMessageContainerView(options: MessageContainerViewOptions) -> MessageContainerViewType {
        makeMessageContainerView(
            channel: options.channel,
    ...
    public func makeMessageTextView(options: MessageTextViewOptions) -> MessageTextViewType {
        makeMessageTextView(
            for: options.message,
    ...
    public func makeMessageDateView(options: MessageDateViewOptions) -> MessageDateViewType {
        makeMessageDateView(for: options.message)
    }

    public func makeMessageAuthorAndDateView(options: MessageAuthorAndDateViewOptions) -> MessageAuthorAndDateViewType {
        makeMessageAuthorAndDateView(for: options.message)
    }

    public func makeMessageTranslationFooterView(options: MessageTranslationFooterViewOptions) -> MessageTranslationFooterViewType {
        makeMessageTranslationFooterView(messageViewModel: options.messageViewModel)
    }

    public func makeLastInGroupHeaderView(options: LastInGroupHeaderViewOptions) -> LastInGroupHeaderView {
        makeLastInGroupHeaderView(for: options.message)
    }

    public func makeImageAttachmentView(options: ImageAttachmentViewOptions) -> ImageAttachmentViewType {
        makeImageAttachmentView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeGiphyAttachmentView(options: GiphyAttachmentViewOptions) -> GiphyAttachmentViewType {
        makeGiphyAttachmentView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeLinkAttachmentView(options: LinkAttachmentViewOptions) -> LinkAttachmentViewType {
        makeLinkAttachmentView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeFileAttachmentView(options: FileAttachmentViewOptions) -> FileAttachmentViewType {
        makeFileAttachmentView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeVideoAttachmentView(options: VideoAttachmentViewOptions) -> VideoAttachmentViewType {
        makeVideoAttachmentView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeGalleryView(options: GalleryViewOptions) -> GalleryViewType {
        makeGalleryView(
    ...
    public func makeGalleryHeaderView(options: GalleryHeaderViewOptions) -> GalleryHeaderViewType {
        makeGalleryHeaderView(title: options.title, subtitle: options.subtitle, shown: options.shown)
    }

    public func makeVideoPlayerView(options: VideoPlayerViewOptions) -> VideoPlayerViewType {
        makeVideoPlayerView(
    ...
    public func makeVideoPlayerHeaderView(options: VideoPlayerHeaderViewOptions) -> VideoPlayerHeaderViewType {
        makeVideoPlayerHeaderView(title: options.title, subtitle: options.subtitle, shown: options.shown)
    }

    public func makeVideoPlayerFooterView(options: VideoPlayerFooterViewOptions) -> VideoPlayerFooterViewType {
        makeVideoPlayerFooterView(attachment: options.attachment, shown: options.shown)
    }

    public func makeDeletedMessageView(options: DeletedMessageViewOptions) -> DeletedMessageViewType {
        makeDeletedMessageView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth
        )
    ...
    public func makeSystemMessageView(options: SystemMessageViewOptions) -> SystemMessageViewType {
        makeSystemMessageView(message: options.message)
    }

    public func makeEmojiTextView(options: EmojiTextViewOptions) -> EmojiTextViewType {
        makeEmojiTextView(message: options.message, scrolledId: options.scrolledId, isFirst: options.isFirst)
    }

    public func makeVoiceRecordingView(options: VoiceRecordingViewOptions) -> VoiceRecordingViewType {
        makeVoiceRecordingView(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeCustomAttachmentViewType(options: CustomAttachmentViewTypeOptions) -> CustomAttachmentViewType {
        makeCustomAttachmentViewType(
            for: options.message,
            isFirst: options.isFirst,
            availableWidth: options.availableWidth,
            scrolledId: options.scrolledId
    ...
    public func makeGiphyBadgeViewType(options: GiphyBadgeViewTypeOptions) -> GiphyBadgeViewType {
        makeGiphyBadgeViewType(for: options.message, availableWidth: options.availableWidth)
    }

    public func makeMessageRepliesView(options: MessageRepliesViewOptions) -> MessageRepliesViewType {
        makeMessageRepliesView(channel: options.channel, message: options.message, replyCount: options.replyCount)
    }

    public func makeMessageRepliesShownInChannelView(options: MessageRepliesShownInChannelViewOptions) -> MessageRepliesShownInChannelViewType {
...
extension ViewFactoryMigration {
    public func makeScrollToBottomButton(options: ScrollToBottomButtonOptions) -> ScrollToBottomButtonType {
        makeScrollToBottomButton(unreadCount: options.unreadCount, onScrollToBottom: options.onScrollToBottom)
    }

    public func makeDateIndicatorView(options: DateIndicatorViewOptions) -> DateIndicatorViewType {
        makeDateIndicatorView(dateString: options.dateString)
    }

    public func makeMessageListDateIndicator(options: MessageListDateIndicatorViewOptions) -> MessageListDateIndicatorViewType {
        makeMessageListDateIndicator(date: options.date)
    }

    public func makeTypingIndicatorBottomView(options: TypingIndicatorBottomViewOptions) -> TypingIndicatorBottomViewType {
        makeTypingIndicatorBottomView(channel: options.channel, currentUserId: options.currentUserId)
    }
}
...
extension ViewFactoryMigration {
    public func makeMessageComposerViewType(options: MessageComposerViewTypeOptions) -> MessageComposerViewType {
        makeMessageComposerViewType(
    ...
    public func makeLeadingComposerView(options: LeadingComposerViewOptions) -> LeadingComposerViewType {
        makeLeadingComposerView(state: options.state, channelConfig: options.channelConfig)
    }

    public func makeComposerInputView(options: ComposerInputViewOptions) -> ComposerInputViewType {
        makeComposerInputView(
            text: options.text,
    ...
    public func makeComposerTextInputView(options: ComposerTextInputViewOptions) -> ComposerTextInputViewType {
        makeComposerTextInputView(
            text: options.text,
    ...
    public func makeTrailingComposerView(options: TrailingComposerViewOptions) -> TrailingComposerViewType {
        makeTrailingComposerView(
    ...
    public func makeComposerRecordingView(options: ComposerRecordingViewOptions) -> ComposerRecordingViewType {
        makeComposerRecordingView(viewModel: options.viewModel, gestureLocation: options.gestureLocation)
    }

    public func makeComposerRecordingLockedView(options: ComposerRecordingLockedViewOptions) -> ComposerRecordingLockedViewType {
        makeComposerRecordingLockedView(viewModel: options.viewModel)
    }

    public func makeComposerRecordingTipView(options: ComposerRecordingTipViewOptions) -> ComposerRecordingTipViewType {
        makeComposerRecordingTipView()
    }

    public func makeComposerViewModifier(options: ComposerViewModifierOptions) -> ComposerViewModifier {
        makeComposerViewModifier()
    }

    public func makeAttachmentPickerView(options: AttachmentPickerViewOptions) -> AttachmentPickerViewType {
        makeAttachmentPickerView(
            attachmentPickerState: options.attachmentPickerState,
    ...
    public func makeAttachmentSourcePickerView(options: AttachmentSourcePickerViewOptions) -> AttachmentSourcePickerViewType {
    ...
    public func makePhotoAttachmentPickerView(options: PhotoAttachmentPickerViewOptions) -> PhotoAttachmentPickerViewType {
        makePhotoAttachmentPickerView(
    ...
    public func makeFilePickerView(options: FilePickerViewOptions) -> FilePickerViewType {
        makeFilePickerView(filePickerShown: options.filePickerShown, addedFileURLs: options.addedFileURLs)
    }

    public func makeCameraPickerView(options: CameraPickerViewOptions) -> CameraPickerViewType {
        makeCameraPickerView(
            selected: options.selected,
            cameraPickerShown: options.cameraPickerShown,
            cameraImageAdded: options.cameraImageAdded
        )
    ...
    public func makeCustomAttachmentView(options: CustomComposerAttachmentViewOptions) -> CustomComposerAttachmentViewType {
        makeCustomAttachmentView(
            addedCustomAttachments: options.addedCustomAttachments,
            onCustomAttachmentTap: options.onCustomAttachmentTap
        )
    }

    public func makeCustomAttachmentPreviewView(options: CustomAttachmentPreviewViewOptions) -> CustomAttachmentPreviewViewType {
        makeCustomAttachmentPreviewView(
            addedCustomAttachments: options.addedCustomAttachments,
            onCustomAttachmentTap: options.onCustomAttachmentTap
        )
    }

    public func makeAssetsAccessPermissionView(options: AssetsAccessPermissionViewOptions) -> AssetsAccessPermissionViewType {
        makeAssetsAccessPermissionView()
    }
}

// MARK: - Message Actions, Reactions & Quoting
...
extension ViewFactoryMigration {
    ...
    public func makeSendInChannelView(options: SendInChannelViewOptions) -> SendInChannelViewType {
        makeSendInChannelView(showReplyInChannel: options.showReplyInChannel, isDirectMessage: options.isDirectMessage)
    }

    public func makeMessageActionsView(options: MessageActionsViewOptions) -> MessageActionsViewType {
        makeMessageActionsView(
            for: options.message,
            channel: options.channel,
            onFinish: options.onFinish,
            onError: options.onError
    ...
    public func makeReactionsUsersView(options: ReactionsUsersViewOptions) -> ReactionsUsersViewType {
        makeReactionsUsersView(message: options.message, maxHeight: options.maxHeight)
    }

    public func makeBottomReactionsView(options: ReactionsBottomViewOptions) -> ReactionsBottomViewType {
        makeBottomReactionsView(
            message: options.message,
            showsAllInfo: options.showsAllInfo,
            onTap: options.onTap,
            onLongPress: options.onLongPress
    ...
    public func makeMessageReactionView(options: MessageReactionViewOptions) -> MessageReactionViewType {
        makeMessageReactionView(
            message: options.message,
    ...
    public func makeReactionsOverlayView(options: ReactionsOverlayViewOptions) -> ReactionsOverlayViewType {
        makeReactionsOverlayView(
            channel: options.channel,
    ...
    public func makeReactionsBackgroundView(options: ReactionsBackgroundOptions) -> ReactionsBackground {
        makeReactionsBackgroundView(
            currentSnapshot: options.currentSnapshot,
            popInAnimationInProgress: options.popInAnimationInProgress
        )
    }

    public func makeReactionsContentView(options: ReactionsContentViewOptions) -> ReactionsContentView {
        makeReactionsContentView(
            message: options.message,
    ...
    public func makeQuotedMessageHeaderView(options: QuotedMessageHeaderViewOptions) -> QuotedMessageHeaderViewType {
        makeQuotedMessageHeaderView(quotedMessage: options.quotedMessage)
    }

    public func makeQuotedMessageView(options: QuotedMessageViewOptions) -> QuotedMessageViewType {
        makeQuotedMessageView(
            quotedMessage: options.quotedMessage,
            fillAvailableSpace: options.fillAvailableSpace,
            isInComposer: options.isInComposer,
            scrolledId: options.scrolledId
        )
    }

    public func makeCustomAttachmentQuotedView(options: CustomAttachmentQuotedViewOptions) -> CustomAttachmentQuotedViewType {
        makeCustomAttachmentQuotedView(for: options.message)
    }

    public func makeEditedMessageHeaderView(options: EditedMessageHeaderViewOptions) -> EditedMessageHeaderViewType {
        makeEditedMessageHeaderView(editedMessage: options.editedMessage)
    }

    public func makeCommandsContainerView(options: CommandsContainerViewOptions) -> CommandsContainerViewType {
        makeCommandsContainerView(suggestions: options.suggestions, handleCommand: options.handleCommand)
    }

    public func makeMessageReadIndicatorView(options: MessageReadIndicatorViewOptions) -> MessageReadIndicatorViewType {
        makeMessageReadIndicatorView(channel: options.channel, message: options.message)
    }

    public func makeNewMessagesIndicatorView(options: NewMessagesIndicatorViewOptions) -> NewMessagesIndicatorViewType {
        makeNewMessagesIndicatorView(newMessagesStartId: options.newMessagesStartId, count: options.count)
    }

    public func makeJumpToUnreadButton(options: JumpToUnreadButtonOptions) -> JumpToUnreadButtonType {
        makeJumpToUnreadButton(
            channel: options.channel,
    ...
    public func makeComposerPollView(options: ComposerPollViewOptions) -> ComposerPollViewType {
        makeComposerPollView(channelController: options.channelController, messageController: options.messageController)
    }

    public func makePollView(options: PollViewOptions) -> PollViewType {
        makePollView(message: options.message, poll: options.poll, isFirst: options.isFirst)
    }
}

// MARK: - Threads & Miscellaneous

extension ViewFactoryMigration {
    public func makeThreadDestination(options: ThreadDestinationOptions) -> @MainActor (ChatThread) -> ThreadDestination {
        makeThreadDestination()
    }

    public func makeThreadListItem(options: ThreadListItemOptions<ThreadDestination>) -> ThreadListItemType {
        makeThreadListItem(
    ...
    public func makeNoThreadsView(options: NoThreadsViewOptions) -> NoThreads {
        makeNoThreadsView()
    }

    public func makeThreadsListErrorBannerView(options: ThreadListErrorBannerViewOptions) -> ThreadListErrorBannerView {
        makeThreadsListErrorBannerView(onRefreshAction: options.onRefreshAction)
    }

    public func makeThreadListLoadingView(options: ThreadListLoadingViewOptions) -> ThreadListLoadingView {
        makeThreadListLoadingView()
    }

    public func makeThreadListContainerViewModifier(options: ThreadListContainerModifierOptions) -> ThreadListContainerModifier {
        makeThreadListContainerViewModifier(viewModel: options.viewModel)
    }

    public func makeThreadListHeaderViewModifier(options: ThreadListHeaderViewModifierOptions) -> ThreadListHeaderViewModifier {
        makeThreadListHeaderViewModifier(title: options.title)
    }

    public func makeThreadListHeaderView(options: ThreadListHeaderViewOptions) -> ThreadListHeaderView {
        makeThreadListHeaderView(viewModel: options.viewModel)
    }

    public func makeThreadListFooterView(options: ThreadListFooterViewOptions) -> ThreadListFooterView {
        makeThreadListFooterView(viewModel: options.viewModel)
    }

    public func makeThreadListBackground(options: ThreadListBackgroundOptions) -> ThreadListBackground {
        makeThreadListBackground(colors: options.colors)
    }

    public func makeThreadListItemBackground(options: ThreadListItemBackgroundOptions) -> ThreadListItemBackground {
        makeThreadListItemBackground(thread: options.thread, isSelected: options.isSelected)
    }

    public func makeThreadListDividerItem(options: ThreadListDividerItemOptions) -> ThreadListDividerItem {
        makeThreadListDividerItem()
    }

    public func makeAddUsersView(options: AddUsersViewOptions) -> AddUsersViewType {
        makeAddUsersView(options: options.options, onUserTap: options.onUserTap)
    }
}
