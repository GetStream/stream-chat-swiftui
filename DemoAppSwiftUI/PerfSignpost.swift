...
/// `os_signpost` interval for the launch timing that is otherwise measured by hand:
/// **M-LAUNCH**, process start to the first rendered, non-empty channel list. The
/// channel-open interval (**M-SETTLE**) is declared for parity but not yet wired up.
///
/// Emitted on the `.pointsOfInterest` category, so it appears in Instruments' Points of
/// Interest track under any template. Disabled signposts cost nothing, so this stays
/// compiled in.
///
...
@MainActor
enum PerfSignpost {
    private static let log = OSLog(subsystem: "io.getstream.iOS.DemoAppSwiftUI.perf", category: .pointsOfInterest)
    ...
    static func beginLaunch() {
        ...
        perfLog(String(format: "[perf] M-LAUNCH begin pre_main_ms=%.1f", preMainMilliseconds))
    }
    ...
    static func finishLaunchOnNextRenderedFrame(reason: String) {
        ...
        afterNextRenderedFrame {
            ...
            perfLog(String(format: "[perf] M-LAUNCH end %@ total_ms=%.1f pre_main_ms=%.1f", reason, millisecondsSinceProcessStart, preMainMilliseconds))
        }
    }
    ...
    /// stderr is unbuffered, so the line survives the process being killed right after it.
    static func perfLog(_ line: String) {
