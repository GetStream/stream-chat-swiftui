...
final class AttachmentFilePickerView_Tests: StreamChatTestCase {
    func test_openingContentTypes_default() {
        let picker = AttachmentFilePickerView(onFilesPicked: { _ in })
    ...
    func test_openingContentTypes_allowedLists() {
        ...
        XCTAssertEqual(AttachmentFilePickerView(onFilesPicked: { _ in }).openingContentTypes, [UTType.pdf])

        chatClient.mockedAppSettings = .mock(fileUploadConfig: .mock(allowedMimeTypes: ["audio/mp3"]))
        XCTAssertEqual(AttachmentFilePickerView(onFilesPicked: { _ in }).openingContentTypes, [UTType.mp3])
    }
}
