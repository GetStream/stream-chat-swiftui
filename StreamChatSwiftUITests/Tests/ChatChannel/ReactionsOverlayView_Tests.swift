...
@MainActor class ReactionsOverlayView_Tests: StreamChatTestCase {
    ...
    func test_reactionsOverlayLayout_outgoingMessageSpanningFold_staysOnRight() {
        let layout = ReactionsOverlayLayout(
            bounds: CGRect(x: 0, y: 0, width: 867, height: 635),
            messageFrame: CGRect(x: 0, y: 200, width: 867, height: 270),
            isRightAligned: true,
            divisionRegionFrames: [CGRect(x: 455.5, y: 0, width: 40, height: 669)]
        )

        XCTAssertEqual(layout.availableContentRect, CGRect(x: 495.5, y: 0, width: 371.5, height: 635))
        XCTAssertEqual(layout.contentOffsetX(contentWidth: 371.5), 495.5)
    }

    func test_reactionsOverlayLayout_incomingMessageSpanningFold_staysOnLeft() {
        let layout = ReactionsOverlayLayout(
            bounds: CGRect(x: 0, y: 0, width: 867, height: 635),
            messageFrame: CGRect(x: 0, y: 200, width: 867, height: 270),
        ...
        XCTAssertEqual(layout.availableContentRect, CGRect(x: 0, y: 0, width: 455.5, height: 635))
        XCTAssertEqual(layout.contentOffsetX(contentWidth: 455.5), 0)
    }

    func test_reactionsOverlayLayout_narrowedColumn_preservesMessageTrailingEdge() {
        let layout = ReactionsOverlayLayout(
            bounds: CGRect(x: 0, y: 0, width: 951, height: 635),
            messageFrame: CGRect(x: 0, y: 200, width: 867, height: 270),
            isRightAligned: true,
            divisionRegionFrames: [CGRect(x: 455.5, y: 0, width: 40, height: 669)]
        )

        XCTAssertEqual(layout.contentOffsetX(contentWidth: 300), 567)
    }

    func test_reactionsOverlayLayout_splitDetail_ignoresFoldOutsideItsBounds() {
        let bounds = CGRect(x: 20, y: 0, width: 371.5, height: 635)
        let layout = ReactionsOverlayLayout(
            bounds: bounds,
            messageFrame: CGRect(x: 20, y: 200, width: 371.5, height: 270),
            isRightAligned: false,
            divisionRegionFrames: [CGRect(x: -20, y: 0, width: 40, height: 669)]
        )

        XCTAssertEqual(layout.availableContentRect, bounds)
        XCTAssertEqual(layout.contentOffsetX(contentWidth: 300), 20)
    }

    func test_reactionsOverlayLayout_horizontalFold_selectsMessageSide() {
        for (messageY, expectedBounds) in [
            (100.0, CGRect(x: 0, y: 62, width: 669, height: 393.5)),
            (600.0, CGRect(x: 0, y: 495.5, width: 669, height: 421.5))
        ] {
            let layout = ReactionsOverlayLayout(
                bounds: CGRect(x: 0, y: 62, width: 669, height: 855),
                messageFrame: CGRect(x: 0, y: messageY, width: 669, height: 100),
                isRightAligned: true,
                divisionRegionFrames: [CGRect(x: 0, y: 455.5, width: 669, height: 40)]
            )

            XCTAssertEqual(layout.availableContentRect, expectedBounds)
        }
    }

    func test_reactionsOverlayLayout_withoutFold_preservesSafeAreaAndAlignment() {
        let bounds = CGRect(x: 59, y: 0, width: 734, height: 359)
        for isRightAligned in [false, true] {
            let layout = ReactionsOverlayLayout(
                bounds: bounds,
                messageFrame: CGRect(x: 59, y: 100, width: 734, height: 90),
                isRightAligned: isRightAligned,
                divisionRegionFrames: []
            )

            XCTAssertEqual(layout.availableContentRect, bounds)
            XCTAssertEqual(layout.contentOffsetX(contentWidth: 734), 59)
        }
    }
