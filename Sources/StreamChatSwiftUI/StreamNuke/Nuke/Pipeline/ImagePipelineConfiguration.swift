...
extension ImagePipeline {
    ...
    struct Configuration: @unchecked Sendable {
        ...
        var dataLoader: any DataLoading

        /// Data cache used by the pipeline.
        var dataCache: (any DataCaching)?

        /// Image cache used by the pipeline.
        var imageCache: (any ImageCaching)? {
        ...
        var makeImageDecoder: @Sendable (ImageDecodingContext) -> (any ImageDecoding)? = {
            ImageDecoderRegistry.shared.decoder(for: $0)
        }

        /// Returns `ImageEncoders.Default()` by default.
        var makeImageEncoder: @Sendable (ImageEncodingContext) -> any ImageEncoding = { _ in
        ...
        var isUsingPrepareForDisplay: Bool = false
        ...
        var dataCachePolicy = ImagePipeline.DataCachePolicy.storeOriginalData
        ...
        var isProgressiveDecodingEnabled = false
        ...
        init(dataLoader: any DataLoading = DataLoader()) {
