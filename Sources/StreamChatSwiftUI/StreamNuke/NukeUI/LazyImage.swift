...
struct LazyImage<Content: View>: View {
    ...
    init(url: URL?, @ViewBuilder content: @escaping (LazyImageState) -> Content) {
    ...
    /// }
    /// ```
    init(request: ImageRequest?, @ViewBuilder content: @escaping (LazyImageState) -> Content) {
        self.request = request.map { HashableRequest(request: $0) }
    ...
    // Deprecated in Nuke 11.0
    @available(*, deprecated, message: "Please use init(request:) or init(url).")
    init(source: (any ImageRequestConvertible)?, @ViewBuilder content: @escaping (LazyImageState) -> Content) {
        self.request = source.map { HashableRequest(request: $0.asImageRequest()) }
        self.makeContent = content
    }
    ...
    func onDisappear(_ behavior: DisappearBehavior?) -> Self {
    ...
    func onStart(_ closure: @escaping (ImageTask) -> Void) -> Self {
        map { $0.onStart = closure }
    }

    /// Gets called when the request progress is updated.
    func onPreview(_ closure: @escaping (ImageResponse) -> Void) -> Self {
        map { $0.onPreview = closure }
    }

    /// Gets called when the request progress is updated.
    func onProgress(_ closure: @escaping (ImageTask.Progress) -> Void) -> Self {
        map { $0.onProgress = closure }
    }

    /// Gets called when the requests finished successfully.
    func onSuccess(_ closure: @escaping (ImageResponse) -> Void) -> Self {
    ...
    func onCompletion(_ closure: @escaping (Result<ImageResponse, Error>) -> Void) -> Self {
    ...
    func onCreated(_ configure: ((ImageView) -> Void)?) -> Self {
